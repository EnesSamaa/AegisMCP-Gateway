...
fn test_ed25519_signature_verification_success_and_failure() {
    ...
    let verify_res =
        verify_plugin_signature(payload, &valid_sig.to_bytes(), verifying_key.as_bytes());
    assert!(verify_res.is_ok());

    let mut invalid_sig = valid_sig.to_bytes();
    invalid_sig[0] ^= 0x01;
    let fail_res = verify_plugin_signature(payload, &invalid_sig, verifying_key.as_bytes());
